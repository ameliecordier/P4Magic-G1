...
/**
 * Classe de tests de l'effet Column Disappear
 * Principe de l'effet : un pion joué sur une case portant l'effet fait
 * disparaitre tous les pions de la colonne (incluant celui joué).
 *
 * @author ElementW
 */
public class ColumnDisappearEffectTest {
    ...
    /**
     * Test de ColumnDisappearEffect sur grille vide Vérification de l'état de la
    ...
    /**
     * Test de ColumnDisappearEffect sur grille vide Vérification du nombre de jetons
    ...
    /**
     * Test de ColumnDisappearEffect sur grille pré-remplie Vérification de l'état de
    ...
    public void testColumnDisappearEffectFilledGame() {
        ...
        for (int i = height - 1; i >= 0; --i) {
            assertEquals("La case " + i + " de la 1ere colonne n'est pas vide", -1, aGame.getBoard().getTileIJ(i, 0).getStatus());
        }
        assertTrue("Doit être d'effet column disappear", aGame.getBoard().getTileIJ(height - 3, 0).getEffect() instanceof ColumnDisappearEffect);
        assertTrue(aGame.getCurrentPlayer().getId() != id_player);
